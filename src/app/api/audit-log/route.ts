...
// DELETE: limpiar audit logs del mes actual o rango de fechas
export async function DELETE(req: NextRequest) {
  try {
    const body = await req.json()
    const { modo, fechaInicio, fechaFinal } = body

    if (modo === 'todo') {
      // Borrar todo el mes actual
      const mes = currentMonthKey()
      await writeList(keyForMonth(mes), [])
      return NextResponse.json({ ok: true, mensaje: `Auditoría de ${mes} completamente limpiada` })
    } else if (modo === 'rango') {
      ...
      await writeList(keyForMonth(mes), filtrado)
      const eliminados = data.length - filtrado.length
      return NextResponse.json({
        ok: true,
        mensaje: `Se eliminaron ${eliminados} registros de auditoría entre ${fechaInicio} y ${fechaFinal}`
      })
    } else {
      return NextResponse.json({ error: 'Modo no válido' }, { status: 400 })
    }
  } catch (err) {
    console.error('Error limpiando auditoría:', err)
