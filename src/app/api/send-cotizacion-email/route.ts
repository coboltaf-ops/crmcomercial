...
export async function POST(req: Request) {
  ...
  let to = '', asunto = '', mensaje = '', cotizacion: any = {}
  try {
    ...
    doc.setFontSize(18)
    doc.text('COTIZACIÓN', 14, 16)
    doc.setFontSize(12)
    doc.text(cotizacion.codigo, 14, 26)
    doc.setFontSize(10)
    doc.text(`Fecha: ${cotizacion.fecha_emision}`, 150, 16)
    doc.text(`Vence: ${cotizacion.fecha_vencimiento}`, 150, 22)

    doc.setTextColor(0, 0, 0)
    doc.setFontSize(10)
    let y = 40
    doc.text(`Empresa: ${cotizacion.cliente_nombre}`, 14, y)
    doc.text(`Contacto: ${cotizacion.contacto_nombre || '—'}`, 14, y + 6)
    doc.text(`Condición de Pago: ${cotizacion.condicion_pago}`, 14, y + 12)
    doc.text(`Moneda: ${cotizacion.tipo_moneda || 'Pesos Colombianos'}`, 14, y + 18)
    doc.text(`Vendedor: ${cotizacion.vendedor || cotizacion.responsable}`, 120, y)
      ...
      <div style="font-family:Arial;max-width:600px;margin:0 auto">
        ...
        <div style="padding:20px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 10px 10px">
          ...
          <table style="width:100%;margin-bottom:16px">
            ...
            <tr><td style="color:#6b7280;padding:4px 0">Fecha:</td><td>${cotizacion.fecha_emision}</td></tr>
            <tr><td style="color:#6b7280;padding:4px 0">Vencimiento:</td><td>${cotizacion.fecha_vencimiento}</td></tr>
        ...
        <p style="text-align:center;color:#9ca3af;font-size:11px;margin-top:16px">CRM Comercial</p>
      </div>`
