...
const emptyCotizacion = (codigo: string, nro: number, responsable: string): Cotizacion => ({
  ...
  situacion: 'Borrador', responsable, vendedor: '', fecha_registro: today, seguimientos: [],
})
  ...
  const handleSendEmail = async () => {
    ...
    try {
      const res = await fetch('/api/send-cotizacion-email', {
        ...
        body: JSON.stringify({ to: emailTo, asunto: emailAsunto || `Cotización ${emailModal.codigo}`, mensaje: emailMsg, cotizacion: emailModal }),
      })
  ...
  const generatePDF = (cot: Cotizacion) => {
    ...
    const html = `<!DOCTYPE html><html><head><title>Cotización ${cot.codigo}</title>
      ...
      <div style="background:#f9fafb;padding:16px;border-radius:8px;margin-bottom:24px;border:1px solid #e5e7eb">
        <p style="color:#1e3a8a;font-size:11px;font-weight:700;margin-bottom:8px;font-weight:700">DATOS DE LA EMPRESA</p>
        ...
        <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: 16, padding: 24, border: '1px solid rgba(255,255,255,0.15)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 16, marginBottom: 20 }}>
            {[
              ...
              { l: 'Contacto', v: viewDetail.contacto_nombre }, { l: 'Condición Pago', v: viewDetail.condicion_pago },
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginBottom: 20 }}>
            ...
            <div>
              ...
              <select value={selected.oportunidad_id} onChange={e => {
                const opo = oposDelCliente.find(o => o.id === e.target.value)
                setSelected({ ...selected, oportunidad_id: e.target.value, oportunidad_nombre: opo?.nombre || '' })
              }} style={inputStyle}>
                <option value="">Ninguna</option>
                {oposDelCliente.map(o => <option key={o.id} value={o.id}>{o.nombre}</option>)}
              </select>
            </div>
            <div>
                    ...
                    <tr key={c.id} style={{ background: i % 2 === 0 ? 'rgba(255,255,255,0.03)' : 'transparent' }}>
                      ...
                      <td style={{ padding: '10px 14px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                        <div style={{ display: 'flex', gap: 6 }}>
                          ...
                          {permisos.eliminar && c.situacion === 'Borrador' && (
                            <button onClick={() => { if (confirm(`¿Eliminar ${c.codigo}?`)) deleteCotizacion(c.id) }} style={{ ...btnStyle, padding: '4px 12px', fontSize: 11, background: '#b91c1c', color: '#ffffff', border: '1px solid #dc2626' }}>Eliminar</button>
                          )}
                        </div>
                      </td>
                    </tr>
