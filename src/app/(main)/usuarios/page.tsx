...
export default function UsuariosPage() {
  ...
  // Cargar usuarios y roles desde KV (servidor)
  useEffect(() => {
    loadUsuarios()
    loadRoles()
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
            {roles.map(r => (
              <button key={r.id} onClick={() => setSelectedRolId(r.id)}
                style={{ ...btnStyle, background: '#ffffff', color: '#013978', border: selectedRolId === r.id ? '3px solid #1d4ed8' : '1px solid #1e3a8a', fontSize: 14, fontWeight: 800 }}>
                {r.nombre}
                {r.nombre !== 'Admin' && (
                  <b style={{ marginLeft: 6, fontSize: 11, fontWeight: 600, color: '#013978' }}>({usuarios.filter(u => u.rol === r.nombre).length})</b>
                )}
              </button>
            ))}
            {!showNewRol ? (
