...
const STORAGE_KEY = 'crm-correos-log'

function getCorreosFromStorage(): CorreoLog[] {
  try {
    const raw = localStorage.getItem(STORAGE_KEY)
    if (!raw) return []
    const parsed = JSON.parse(raw)
    return parsed.state?.correos || parsed.correos || parsed || []
  } catch { return [] }
}

function saveCorreosToStorage(correos: CorreoLog[]) {
  localStorage.setItem(STORAGE_KEY, JSON.stringify({ state: { correos }, version: 0 }))
}
...
export default function CorreosPage() {
  ...
  const cargar = useCallback(() => {
    setLoading(true)
    const data = getCorreosFromStorage()
    setCorreos(data)
    setLoading(false)
  }, [])
